...
@manage_profile_blueprint.route('/manage_profile', methods=['GET', 'PUT'])
def update_user():
    db = get_users_db()
    ...
    try:
        ...
        if not user_id:
            return render_template('manageProfile.html', error='Unauthorized')

        user = users_collection.find_one({'_id': ObjectId(user_id)})
        if not user:
            return render_template('manageProfile.html', error='User not found')
